            ...
            <script>
                async function fetchQR() {
                    ...
                    if (data.status === 'ready') {
                        ...
                        document.getElementById('status').innerText = 'Scan QR ini menggunakan WhatsApp (Ganti otomatis tiap ~60dtk)';
                    } else {
                        document.getElementById('status').innerText = 'Status: ' + data.status + '...';
                    }
                }
                fetchQR();
                setInterval(fetchQR, 5000); // Polling update tiap 5 detik
            </script>
        </body></html>
    `);
});
