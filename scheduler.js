...
async function sendWhatsAppMessage(client, target, message) {
    try {
        const chatId = `${target}@c.us`;
        await client.sendMessage(chatId, message);
        console.log(`[WA] Berhasil mengirim ke ${target}`);
    } catch (error) {
        console.error(`[WA] Gagal mengirim pesan ke ${target}:`, error.message);
    }
}
    ...
    try {
        ...
        if (tasksToRun.length > 0) {
            ...
            for (const task of tasksToRun) {
                ...
                await sendWhatsAppMessage(client, targetNumber, textMsg);
